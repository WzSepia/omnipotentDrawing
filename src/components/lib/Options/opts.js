...
export function setOpts(opts) {
    ...
    const style = {
      ...
      outerWidth: opts.style.truncate.outerWidth || null, //	number	null	包含了 textPadding 的宽度，超出这个范围就裁剪。
      outerHeight: opts.style.truncate.outerHeight || null, //	number	null	包含了 textPadding 的高度，超出这个范围就裁剪。
      ellipsis: opts.style.truncate.ellipsis || '...', //	strinsg	'...'	默认用省略号表示超出部分，也可以对其进行自定义。
      placeholder: opts.style.truncate.placeholder || null, //	string	null	如果空间过小，导致省略号也显示不下，但是又不想空着，可能得有个什么标记表示这里是有字符的，就用个 “点”，就是在这个 placeholder 里设置。
      blend: opts.style.blend || null, //	string	null	混合模式，同 Canvas globalCompositeOperation。
    };
    return Object.assign({},init,style);
}